...
def post_save_game(sender, created, instance, **kwargs):
    '''
    Signal to auto create next game based off matchup and change session status
    '''
    session = instance.session
    session.status = 2
    session.save()
    seq_inc = Game.objects.filter(
        session=session, creation_type='sequence'
        ).count()
    ...
    if instance.creation_type == 'sequence':
        fixture = Matchup.objects.get(
            player_count=session.player_count, game_index=game_i
            )
        roster = session.session_roster.all().order_by('roster')
        team1 = Team.objects.filter(
            team_players=roster[fixture.team1_player1_index - 1].player
            ).filter(
                team_players=roster[fixture.team1_player2_index - 1].player
                )
        team2 = Team.objects.filter(
            team_players=roster[fixture.team2_player1_index - 1].player
            ).filter(
                team_players=roster[fixture.team2_player2_index - 1].player
                )

        if not team1.exists():
            new_team = Team.objects.create()
            new_team.team_players.set(
                [roster[fixture.team1_player1_index - 1].player,
        ...
        if not team2.exists():
            ...
            new_team2.team_players.set(
                [roster[fixture.team2_player1_index - 1].player,
